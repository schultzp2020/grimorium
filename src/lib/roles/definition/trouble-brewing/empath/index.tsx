...
const empathNightSteps: NightStepDefinition[] = [
  {
...
function EmpathNightAction({ state, player, onComplete }: NightActionProps) {
  ...
  const ambiguousPlayers = useMemo(() => getAmbiguousPlayers(neighbors, 'alignment'), [neighbors])
  ...
  return (
    <PlayerFacingScreen playerName={player.name}>
      <TeamBackground teamId={resultTeam}>
        <OracleCard icon='handHeart' teamId={resultTeam} title={roleT.info} subtitle={getRoleName('empath', language)}>
          <NumberReveal value={displayedEvilNeighbors} label={roleT.evilNeighborsCount} teamId={resultTeam} />
        </OracleCard>
...
export default defineRole({
  ...
  nightSteps: empathNightSteps,
  NightAction: EmpathNightAction,
})
