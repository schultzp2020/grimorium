...
export function NumberReveal({ value, label, teamId }: NumberRevealProps) {
  ...
  return (
    <div className="text-center py-2 sm:py-4">
      {/* Label */}
      <p className={cn("text-sm tracking-wide mb-3 sm:mb-6 opacity-80", team.colors.cardText)}>
        {label}
      </p>
    ...
    <div className="text-center py-2 sm:py-4">
      ...
      <div className="space-y-2 mb-4 sm:mb-6">
        {players.map((name, i) => (
          <div
            key={i}
            className={cn("px-4 py-2.5 rounded-lg border text-center", team.colors.cardWinBg)}
          >
            <span
              className={cn("font-tarot text-lg tracking-wide uppercase", team.colors.cardText)}
            >
              {name}
            </span>
          </div>
