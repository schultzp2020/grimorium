...
export function RoleSelection({ players, scriptId, onNext, onBack }: Props) {
  ...
  const [presetPools, setPresetPools] = useState<Record<GeneratorPreset, GeneratedPool> | null>(
    null,
  );
  ...
  return (
    <div className="min-h-app bg-gradient-to-b from-grimoire-purple via-grimoire-dark to-grimoire-darker flex flex-col">
      ...
      {mode === "manual" && recommended && (
        <DistributionTracker recommended={recommended} teamCounts={teamCounts} />
      )}
    ...
    <div className="px-4 py-4 max-w-lg mx-auto w-full">
      ...
      {presetPools && activePool && poolRolesByTeam ? (
        ...
        >
          {/* Chaos score header */}
          <div
            className={cn("px-4 py-2.5 flex items-center justify-between", activeConfig.bgColor)}
          >
            <span
              className={cn(
    ...
    <div>
      ...
      <div className="sticky top-0 z-[5] bg-grimoire-dark/95 backdrop-blur-xs border-b border-white/[0.06] px-4 py-2">
        <div className="max-w-lg mx-auto flex items-center gap-2">
          ...
          {isMatch && <Icon name="check" size="xs" className="text-green-400 ml-auto" />}
          {isOver && <Icon name="alertTriangle" size="xs" className="text-amber-400 ml-auto" />}
        </div>
      </div>
    ...
    >
      ...
      {isSelected && isCustomMode && (
        ...
        >
          ...
          <span className={cn("text-sm font-medium min-w-[1.5rem] text-center", team.colors.text)}>
            {count}
          </span>
          <button
            type="button"
