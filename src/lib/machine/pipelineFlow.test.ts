...
describe('pipeline needs_input flow', () => {
  ...
  it('transitions to pipeline_input when intent returns needs_input', () => {
    ...
    expect(snapshot.matches({ playing: { night: 'pipeline_input' } })).toBe(true)
  ...
  it('resolves pipeline and returns to dashboard on PIPELINE_INPUT_COMPLETE', () => {
    const MockComponent = () => null
    const resumeFn = vi.fn().mockReturnValue({
      type: 'resolved',
      stateChanges: {
        entries: [
          {
    ...
    expect(actor.getSnapshot().matches({ playing: { night: 'pipeline_input' } })).toBe(true)
  ...
  it('stays in pipeline_input when resume returns needs_input again', () => {
    ...
    const secondResumeFn = vi.fn().mockReturnValue({
      type: 'resolved',
      stateChanges: { entries: [] } as StateChanges,
    })

    const firstResumeFn = vi.fn().mockReturnValue({
      type: 'needs_input',
