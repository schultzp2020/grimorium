...
export function RoleCard({ roleId }: Props) {
  ...
  if (!role) {
    return <p className='p-4 text-center font-tarot text-red-400'>{interpolate(t.ui.unknownRoleId, { roleId })}</p>
  }
  ...
  return (
    <CardShell teamId={teamId} icon={role.icon}>
      ...
      <div className='flex flex-col items-center pt-4'>
        ...
        {/* Team Badge */}
        <p className={cn('text-center text-xs tracking-widest uppercase', team.colors.cardTeamBadge)}>{teamName}</p>
      </div>

      {/* Middle: Divider + Quote + Lines — centered in remaining space */}
      <div className='flex flex-1 flex-col items-center justify-center p-4 sm:p-2'>
