    ...
    <div className='flex min-h-app flex-col bg-gradient-to-b from-red-950 via-grimoire-blood to-grimoire-darker'>
      ...
      <div className='bg-gradient-to-b from-red-900/50 to-transparent px-4 py-4'>
        <div className='mx-auto max-w-lg'>
          ...
          <div className='mb-4 flex items-center'>
            ...
            <span className='ml-1 text-xs text-parchment-500'>{t.game.cancelNomination}</span>
          </div>
          ...
          <div className='text-center'>
            ...
            <p className='text-sm text-parchment-400'>{interpolate(t.game.votesNeeded, { count: threshold })}</p>
          </div>
        </div>
      </div>
      ...
      <div className='mx-auto w-full max-w-lg px-4'>
        <div className='flex justify-around border-b border-white/10 py-4'>
          <div className='text-center'>
            <div className={cn('text-3xl font-bold', meetsThreshold ? 'text-green-400' : 'text-red-400')}>
              {voteCount}
            </div>
            <div className='text-xs tracking-wider text-parchment-400/70 uppercase'>{t.game.votesCount}</div>
          </div>
          <div className='text-center'>
            <div className='text-3xl font-bold text-parchment-500'>{threshold}</div>
            <div className='text-xs tracking-wider text-parchment-500/70 uppercase'>
      ...
      {blockStatus && (
        <div className='mx-auto w-full max-w-lg px-4'>
          <div className='mt-3 rounded-lg border border-amber-500/30 bg-amber-900/20 p-3'>
            ...
            <p className='mt-1 ml-6 text-xs text-amber-400/60'>
      ...
      {/* Vote Input Area */}
      <div className='mx-auto w-full max-w-lg flex-1 overflow-y-auto px-4 py-3 pb-4'>
            ...
            return (
              ...
              >
                <div className='mb-2 flex items-center gap-2'>
                  ...
                  <span className='flex-1 text-sm text-parchment-200'>{player.name}</span>
                  {isNominee && <span className='pr-1 text-xs text-red-400/80 italic'>{t.game.nominee}</span>}
                  {isDead && !ghostVoteSpent && canVote && !isNominee && (
                    <span className='inline-flex items-center gap-1 rounded-sm bg-blue-900/20 px-1.5 py-0.5 text-xs text-blue-400/80'>
                      <Icon name='ghost' size='xs' />
                      {t.game.ghostVoteAvailable}
                    </span>
                  )}
                  {ghostVoteSpent && (
                    <span className='inline-flex items-center gap-1 rounded-sm bg-white/5 px-1.5 py-0.5 text-xs text-parchment-500/50'>
                      <Icon name='ghost' size='xs' />
      ...
      <div className='mx-auto w-full max-w-lg px-4'>
        ...
        >
          {outcome === 'goes_on_block' && (
            <p className='text-sm text-green-200'>{interpolate(t.game.goesOnBlock, { player: nominee.name })}</p>
          )}
          {outcome === 'replaces_block' && (
            <p className='text-sm text-green-200'>{interpolate(t.game.goesOnBlock, { player: nominee.name })}</p>
          )}
          {outcome === 'tied' && <p className='text-sm text-amber-200'>{t.game.tiedNoExecution}</p>}
          {(outcome === 'not_enough' || outcome === 'below_block') && (
            <p className='text-sm text-red-200'>{t.game.notEnoughVotes}</p>
          )}
        </div>
      </div>
