...
function App() {
  ...
  const handleStartGame = (roleAssignments: { name: string; roleId: string }[], scriptId: ScriptId) => {
  ...
  if (routeType === 'roles') {
    ...
    const selectedRoleId = candidateRoleId && getRole(candidateRoleId as RoleId) ? (candidateRoleId as RoleId) : null
        ...
        case 'new_game_roles': {
          return (
            <RoleSelection
              ...
              onNext={(selectedRoles) => handleRolesNext(newGameScreen.players, newGameScreen.scriptId, selectedRoles)}
              onBack={() =>
                setNewGameScreen({
