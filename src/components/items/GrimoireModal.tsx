...
export function GrimoireModal({
  ...
  const isDirectPlayerOpen = intent.view === 'player_detail' && view.type === 'player_detail'
      ...
      <DialogContent className='max-h-[90vh]'>
        <DialogHeader>
          <div className='flex items-center gap-3'>
            {showBackButton && <BackButton onClick={goBack} label={t.common.back} />}
            <div className='flex flex-1 items-center justify-center gap-2'>
              {view.type === 'list' && <Icon name='bookUser' size='md' className='text-mystic-gold' />}
              <DialogTitle className={showBackButton ? 'text-left' : ''}>{getHeaderTitle()}</DialogTitle>
            </div>
          </div>
        </DialogHeader>
        <DialogBody>
          ...
          {view.type === 'player_detail' && (
            <PlayerDetailContent
              player={view.player}
              showEditEffects={!('readOnly' in intent && intent.readOnly === true)}
              onShowRoleCard={() => {
                handleClose()
          ...
          {view.type === 'effect_config' && (
            <EffectConfigContent
              ...
              onCancel={() => setView({ type: 'edit_effects', player: view.player })}
            />
          )}
        </DialogBody>
      </DialogContent>
                ...
                >
                  ...
                  {DescriptionComponent ? (
                    ...
                    <p className='mt-2 text-xs text-parchment-400'>
                      {getRegistryEffectDescription(effectInstance.type, language)}
                    </p>
                  )}
                </div>
              )
...
function EditEffectsContent({
  ...
  onRemoveEffect: (playerId: string, effectType: string) => void
  onBack: () => void
  onOpenConfig: (effectDef: EffectDefinition, mode: 'add' | 'edit', effectInstance?: EffectInstance) => void
}) {
  const { t, language } = useI18n()
  ...
  return (
    <div className='space-y-6'>
      <p className='text-center text-sm text-parchment-400'>{player.name}</p>

      <div>
        <div className='mb-3 flex items-center gap-2'>
          <Icon name='sparkles' size='sm' className='text-cyan-400' />
          <span className='font-tarot text-sm tracking-wider text-parchment-100 uppercase'>{t.ui.currentEffects}</span>
        </div>
          ...
          {allEffects.map((effect) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className={cn('text-sm flex-1', alreadyHas ? 'text-parchment-500' : 'text-parchment-200')}>
                  {effectName}
                </span>
