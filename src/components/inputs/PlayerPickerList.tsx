...
export function PlayerPickerList({
  ...
  const renderPlayer = (player: PlayerState) => {
    ...
    const isDisabled =
      (!isSelected && isAtMax && selectionCount !== 1) || (disabledSet?.has(player.id) ?? false);
    ...
    return (
      ...
      >
        <PlayerRoleIcon
          ...
          size="sm"
          circleClassName={
            isSelected && team ? team.colors.cardIconBg : "bg-white/5 border border-white/10"
          }
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            {role && team && (
              <>
                ...
                <span
                  className={cn("text-xs", team.isEvil ? "text-red-400/70" : "text-blue-300/70")}
                >
          ...
          {annotations?.[player.id] && (
            <div className="text-xs text-parchment-600 mt-0.5 italic">{annotations[player.id]}</div>
          )}
        </div>
