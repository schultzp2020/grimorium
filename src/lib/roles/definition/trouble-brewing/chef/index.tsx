...
export function countEvilPairs(state: GameState, observer: PlayerState): number {
  ...
  for (let i = 0; i < aliveIndices.length; i++) {
    ...
    const currentIsEvil = perceive(currentPlayer, observer, 'alignment', state).alignment === 'evil'
...
const chefNightSteps: NightStepDefinition[] = [
  {
  ...
  },
  {
    ...
    condition: (_game, player, state) =>
      !isMalfunctioning(player) && getAmbiguousPlayers(state.players.filter(isAlive), 'alignment').length > 0,
    audience: 'narrator',
  },
  {
    id: 'show_result',
...
function ChefNightAction({ state, player, onComplete }: NightActionProps) {
  ...
  return (
    <PlayerFacingScreen playerName={player.name}>
      <TeamBackground teamId={resultTeam}>
        <OracleCard icon='chefHat' teamId={resultTeam} title={roleT.info} subtitle={getRoleName('chef', language)}>
          <NumberReveal value={displayedEvilPairs} label={roleT.evilPairsCount} teamId={resultTeam} />
        </OracleCard>
...
export default defineRole({
  ...
  nightSteps: chefNightSteps,
  NightAction: ChefNightAction,
})
