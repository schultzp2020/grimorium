...
export function PlayerPickerList({
  ...
  const renderPlayer = (player: PlayerState) => {
    ...
    const isDisabled = (!isSelected && isAtMax && selectionCount !== 1) || (disabledSet?.has(player.id) ?? false)
    ...
    return (
      <button
        ...
        className={cn(
          ...
          isSelected ? styles.selected : 'border-white/10 bg-white/5 hover:bg-white/[0.08]',
      ...
      >
        <PlayerRoleIcon
          player={player}
          size='sm'
          circleClassName={isSelected && team ? team.colors.cardIconBg : 'bg-white/5 border border-white/10'}
          iconClassName={isSelected && team ? team.colors.text : 'text-parchment-500'}
        />
        ...
        <div className='min-w-0 flex-1 text-left'>
          ...
          <span
            className={cn(
              ...
              isDead ? 'text-parchment-500 line-through' : isSelected ? 'text-parchment-100' : 'text-parchment-200',
            )}
          >
            {player.name}
          </span>
          ...
          <div className='mt-0.5 flex items-center gap-1.5'>
            {role && team && (
              <>
                <span className={cn('text-xs', team.colors.text)}>{getRoleName(role.id)}</span>
                <span className='text-parchment-600'>&middot;</span>
                <span className={cn('text-xs', team.isEvil ? 'text-red-400/70' : 'text-blue-300/70')}>
          ...
          {annotations?.[player.id] && (
            <div className='text-parchment-600 mt-0.5 text-xs italic'>{annotations[player.id]}</div>
          )}
        </div>
        ...
        {isSelected && (
          ...
          >
            <Icon name='check' size='xs' className={styles.selectedAccent} />
          </div>
        )}
      </button>
    )
        ...
        {groups.map((group) => {
          ...
          return (
            <div key={group.label} className='space-y-2'>
              <div className='mb-1 ml-1 font-tarot text-xs tracking-wider text-parchment-400/80 uppercase'>
                {group.label}
              </div>
              <div className='space-y-2'>{groupPlayers.map(renderPlayer)}</div>
            </div>
          )
        })}
      </div>
