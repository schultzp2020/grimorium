 ...
 */
export function OracleCard({ icon, teamId, title, subtitle, children }: OracleCardProps) {
  ...
  return (
    <CardShell teamId={teamId} icon={icon}>
      ...
      {/* Subtitle (role name) */}
      <p className={cn('text-center text-xs tracking-widest uppercase mb-3 sm:mb-6', team.colors.cardTeamBadge)}>
        {subtitle}
      </p>

      {/* Divider — team-specific icon */}
      <MysticDivider icon={team.colors.cardDividerIcon} iconClassName={cn(team.colors.cardWinAccent, 'opacity-50')} />
...
export function NumberReveal({ value, label, teamId }: NumberRevealProps) {
  ...
  return (
    <div className='py-2 text-center sm:py-4'>
      {/* Label */}
      <p className={cn('text-sm tracking-wide mb-3 sm:mb-6 opacity-80', team.colors.cardText)}>{label}</p>
      ...
      >
        <span
          className={cn('font-tarot text-5xl sm:text-7xl font-bold', team.colors.cardWinAccent)}
          style={{ textShadow: team.colors.cardIconGlow }}
        >
          {value}
        </span>
 ...
 */
export function VisionReveal({ players, verdict, verdictIcon, teamId }: VisionRevealProps) {
  const team = getTeam(teamId)
  ...
  return (
    <div className='py-2 text-center sm:py-4'>
      ...
      <div className='mb-4 space-y-2 sm:mb-6'>
        {players.map((name, i) => (
          <div key={i} className={cn('px-4 py-2.5 rounded-lg border text-center', team.colors.cardWinBg)}>
            <span className={cn('font-tarot text-lg tracking-wide uppercase', team.colors.cardText)}>{name}</span>
          </div>
        ))}
      </div>
      ...
      <div className='flex flex-col items-center gap-2'>
        <Icon name={verdictIcon} size='xl' className={team.colors.cardWinAccent} />
        <p
          className={cn('font-tarot text-lg sm:text-xl uppercase tracking-wider leading-relaxed', team.colors.cardText)}
          style={{ textShadow: team.colors.cardIconGlow }}
        >
          {verdict}
        </p>
